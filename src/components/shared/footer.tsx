...
import { DataBot } from '@/types/settings'
...
const Footer = () => {
  const { Footer } = useStore(useSettingStore)

  const dataTop = Footer.Top
  const dataBot = Footer.Bot
  return (
                  ...
                  'Links' in value && (
                    <div className='flex flex-col'>
                      ...
                      <ul className='flex flex-col gap-5 mt-[30px]'>
                        {value.Links.map(
                          (link: { Id: number; To: string; Name: string }) => (
                            <li key={link.Id}>
                              <a
                                href={link.To}
